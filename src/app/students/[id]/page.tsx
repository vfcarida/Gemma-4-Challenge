...
import { ArrowLeft, Volume2, Sun, Hand, Clock, Layout } from 'lucide-react';
import { getStudentById, getSessionLogsByStudent, getBoards } from '@/lib/storage';
import { ROUTES } from '@/lib/constants';
import { formatDate, getInitials, formatRelativeTime } from '@/lib/utils';
import type { StudentProfile, SessionLog, PECSBoard } from '@/lib/types';
...
export default function StudentDetailPage() {
  ...
  useEffect(() => {
    ...
    if (s) {
      setStudent(s);
      setSessions(getSessionLogsByStudent(studentId));
      setBoards(getBoards().filter((b) => b.studentId === studentId));
    }
  }, [studentId]);
  ...
  return (
    <div className="p-6 lg:p-10 max-w-5xl mx-auto space-y-8">
      ...
      {/* Saved Boards */}
      <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm space-y-4">
        <h2 className="text-lg font-bold text-slate-800">Saved PECS Boards ({boards.length})</h2>
        {boards.length === 0 ? (
      ...
      {/* Session History */}
      <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm space-y-4">
        <h2 className="text-lg font-bold text-slate-800">Session History ({sessions.length})</h2>
        {sessions.length === 0 ? (
